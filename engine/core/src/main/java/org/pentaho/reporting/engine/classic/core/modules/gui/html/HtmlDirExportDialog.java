...
public class HtmlDirExportDialog extends AbstractExportDialog {
  ...
  protected Configuration grabDialogContents( final boolean full ) {
    ...
    if ( full ) {
      p.setConfigProperty( "org.pentaho.reporting.engine.classic.core.modules.gui.html.file.DataDirectory", //$NON-NLS-1$
          getDataFilename() );
  ...
  protected void setDialogContents( final Configuration p ) {
    ...
    setFilename( p.getConfigProperty( "org.pentaho.reporting.engine.classic.core.modules.gui.html.file.TargetFileName",
        "" ) ); //$NON-NLS-1$ //$NON-NLS-2$
  }
  ...
  protected void performSelectFile() {
    ...
    if ( fileChooserHtml == null ) {
      ...
      fileChooserHtml.addChoosableFileFilter( new FilesystemFilter( new String[] {
        HtmlDirExportDialog.HTML_FILE_EXTENSION, HtmlDirExportDialog.HTM_FILE_EXTENSION }, getResources().getString(
          "htmlexportdialog.html-documents" ), true ) ); //$NON-NLS-1$
  ...
  protected boolean performConfirm() {
    ...
    if ( dataDir.exists() == false ) {
      ...
      if ( JOptionPane.showConfirmDialog( this, MessageFormat.format( getResources().getString( dataDirKey1 ),
          new Object[] { getDataFilename() } ), getResources().getString( dataDirKey2 ), JOptionPane.YES_NO_OPTION,
          JOptionPane.QUESTION_MESSAGE ) == JOptionPane.NO_OPTION ) {
        return false;
      }
    }
